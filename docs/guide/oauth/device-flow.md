...
To start the device authorisation OAuth flow, you need to send a `POST` request to `https://directory.zaikio.com/oauth/device_authorizations/authorize`.
...
To start polling you need to send a `POST` request to `https://directory.zaikio.com/oauth/access_token`.
